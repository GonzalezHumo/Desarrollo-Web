<?php
// ── Lógica conversión de segundos (lado servidor) ─────────────
$resultado = null;
$segundos  = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($segundos === '' || !is_numeric($segundos) || (int)$segundos < 0) {
        $resultado = 'error';
    } else {
        $total  = (int)$segundos;
        ...
        $segs   = $resto % 60;
        $resultado = ['h' => $horas, 'm' => $minutos, 's' => $segs, 'total' => $total];
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Gonzalez Humo · Ejercicio 35 — Convertidor de Segundos</title>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .wrap{max-width:480px;width:100%;position:relative;z-index:1;}
    ...
    /* ── Input ── */
    .ig{display:flex;flex-direction:column;gap:7px;margin-bottom:26px;}
    ...
    .result-value{
      font-family:'Playfair Display',serif;
      font-size:1.4rem;font-weight:800;color:#2d7a50;line-height:1.5;
    }
...
<div class="wrap">
  ...
  <!-- Header -->
  <div class="header">
    <div class="badge">Ejercicio 35</div>
    <h1>Convertidor de<br><span>Segundos</span></h1>
    <p>Ingresa un número de segundos para convertirlo en horas, minutos y segundos.</p>
  </div>

  <!-- Error -->
  <?php if ($resultado === 'error'): ?>
    <div class="alert">Por favor ingresa un número de segundos válido.</div>
  <?php endif; ?>
  ...
  <?php if (is_array($resultado)): ?>
    <div class="result-card">
      ...
      <div class="rc-body">
        <div class="result-label"><?= $resultado['total'] ?> segundos</div>
        <div class="result-value">
          <?= $resultado['total'] ?> segundos corresponden a<br>
          <?= $resultado['h'] ?>h, <?= $resultado['m'] ?>m y <?= $resultado['s'] ?>s
        </div>
      </div>
    </div>
  <?php endif; ?>
  ...
  <form method="POST" action="">
    <div class="card">
      <h2>Ingresa los segundos</h2>
      <div class="ig">
        <label>Total de segundos</label>
        <span class="hint">ej. 3661</span>
        <input type="number" name="segundos" min="0" step="1" placeholder=""
               value="<?= htmlspecialchars($segundos) ?>"/>
      </div>
      <button type="submit" class="main-btn">Convertir</button>
    </div>
  </form>
